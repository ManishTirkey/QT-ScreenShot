...
bool GlobalHotkeyFilter::registerHotKey()
{
    // Register Print Screen key as global hotkey
    return RegisterHotKey(NULL, HOTKEY_ID, 0, VK_SNAPSHOT);
}

bool GlobalHotkeyFilter::unregisterHotKey()
{
    // Unregister the hotkey
    return UnregisterHotKey(NULL, HOTKEY_ID);
}
...
#endif
{
    ...
    {
        // Hotkey was pressed, call the target slot
        QMetaObject::invokeMethod(m_target, m_slot);
        return true;
    }
...
void ScreenshotTool::mouseReleaseEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton && isCapturing) {
        ...
        if (QRect(startPos, endPos).normalized().width() > 10 && 
            ...
            // Show the captured image in a window (without copying to clipboard)
            showCapturedImage();
        }
...
void ScreenshotTool::showShortcutsDialog()
{
    QMessageBox::information(nullptr, "Keyboard Shortcuts",
        "Global Shortcuts:\n"
        "- Print Screen: Capture screenshot (works system-wide)\n\n"
        "During Capture:\n"
        "- ESC: Cancel capture\n\n"
        "In Preview Window:\n"
...
void ScreenshotTool::closeEvent(QCloseEvent *event)
{
    ...
    // Hide instead of close when the window is closed
    // This allows the application to continue running in the system tray
